...
document.addEventListener("DOMContentLoaded", function() {
    ...
    form.addEventListener("submit", function(event) {
        ...
        else if (!isValidEmail(field13.value)) { // Check if email is valid
            errorMessageFive.textContent = "Invalid email format"; // Show error message for invalid email format
            showerrorLi();
        } else {
            hideError();
            hideerrorLi();
    ...
    function showerrorLi() {
        errorLi.classList.remove("hidden"); 


    }

    function hideerrorLi() {
        errorLi.classList.add("hidden"); 
...
function isValidEmail(field13.value) {
    return /^[\w\.-]+@[a-zA-Z\d\.-]+\.[a-zA-Z]{2,}$/.test(field13.value);
}
